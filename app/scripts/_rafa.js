'use strict';
	...
	var myRafa = { }, paper;
	    ...
	    var angle = 0,
	        ...
	        process = function (j) {
	            var value = values[j],
	                ...
	                color = Raphael.hsb(start, 0.5, 0.5),
	                ms = 500,
	                delta = 30,
	 ...
	 myRafa.init = function( id,w,h ){
		paper = Raphael(id,w,h);
	}

	myRafa.drawMessage = function(txt,counter,x,y,fSize,i,del){
                if( ++counter < txt[i].length ){
                    ...
                    .animate({opacity: 1}, del, function(){
                        myRafa.drawMessage( txt, counter, x, y, fSize, i, del );
                    });

                }else if( txt[++i] ){
                	myRafa.drawMessage( txt, 0, x, y*3, fSize, i, del);
                }
        };
	
	myRafa.bublik = function( results ){
